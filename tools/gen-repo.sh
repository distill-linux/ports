...
for fname in sorted(os.listdir(repo_dir)):
    ...
    name = meta.get("PORT_NAME") or meta.get("NAME") or fname.split("-")[0]
    version = meta.get("PORT_VERSION") or meta.get("VERSION") or "1.0"
    release = meta.get("PORT_RELEASE") or meta.get("RELEASE") or "1"
    desc = meta.get("PORT_DESC") or meta.get("DESC") or "No description available."
    deps = meta.get("RUN_DEPS") or meta.get("DEPENDS") or ""
    url = meta.get("PORT_URL") or meta.get("URL") or ""
...
for p in packages:
    ...
    row = f'''        <tr class="pkg-row" data-name="{p['name']}" data-desc="{desc_lower}" data-date="2026-09-02" data-size="{p['size']}">
            ...
            <td>
                ...
                <div style="margin-top: 0.3em;"><code class="pkg-cmd">drop in {p['name']}</code></div>
            </td>
            <td class="pkg-meta">distill-core</td>
            <td class="pkg-meta">2026-09-02 (UTC)</td>
            <td>
                <a href="{p['filename']}" download style="color: #aa2022; font-weight: bold;">{p['filename']}</a><br>
